...
def process_files(files: Iterable[Path], vault_password_file: Path, plain_suffix: str) -> list[Path]:
    modified: list[Path] = []
    for path in files:
        changed = process_file(path=path, vault_password_file=vault_password_file, plain_suffix=plain_suffix)
        if changed:
            modified.append(path)
    return modified


def process_file(path: Path, vault_password_file: Path, plain_suffix: str) -> bool:
    yaml = build_yaml()
    ...
    for key, value in list(data.items()):
        ...
        data[key] = encrypt_value(key=key, value=value, vault_password_file=vault_password_file)
        changed = True
...
def encrypt_value(key: str, value: str, vault_password_file: Path):
    command = [
        ansible_vault_bin(),
        "encrypt_string",
        "--vault-password-file",
